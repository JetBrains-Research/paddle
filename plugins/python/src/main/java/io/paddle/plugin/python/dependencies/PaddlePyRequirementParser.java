 ...
 */
public class PaddlePyRequirementParser {
    ...
    @NotNull
    public static List<PyRequirementVersionSpec> parseVersionSpecs(@Nullable String versionSpecs) {
        if (versionSpecs == null) return Collections.emptyList();

        return StreamSupport
                .stream(StringUtil.tokenize(versionSpecs, ",").spliterator(), false)
                .map(String::trim)
                .map(PaddlePyRequirementParser::parseVersionSpec)
                .filter(req -> req != null)
